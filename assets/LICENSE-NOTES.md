# Asset and license notes

This file records what this repository contains and what the available sources say about it, so that
users and rights holders can judge for themselves. It is not legal advice and it is not a license grant.

## 1. Original Mine Bombers game files

**Owner:** Skitso Productions (Finland). Mine Bombers was shareware (1995-96); version 3.11 was later
released as freeware.

**This project does not contain or distribute them.** Like OpenRCT2 with RollerCoaster Tycoon 2, it is an
engine that needs the user's own copy of the game:

- The repository has no original game files, and neither does the deployed site or the WebAssembly module.
- The native program takes the folder of the user's copy (`MineBombers <game folder>`).
- The web page asks the visitor for their copy (ZIP, folder or files). Only the ~30 files the game uses
  are kept, in that visitor's own browser storage, and they are never uploaded anywhere. The files are
  used as they are: nothing is converted or re-encoded, and no derived copy is committed. Sound effects
  are decoded and the music module is played in the browser.
- Tests do not read original files. They run on stand-ins generated by the test code itself. Checks
  against a real copy exist and run only when a developer points `MB_GAME_DIR` / `MB_GAME_ZIP` at one.

**History.** Earlier versions of this project stored the original files: `res/minebomb/` and
`res/minebomb.zip`, converted audio, a JS-DOS bundle, the maps converted to JSON, WebAssembly builds with
the images, font and maps compiled in, and an application icon and `MineBombers.exe` builds carrying an
image cropped from the original title screen. On 2026-09-20 all of it was removed from the entire history,
which was moved to this new repository; the earlier repository has been taken down. Copies that other
people made before that date cannot be recalled.

**What the original package itself says** (files inside the package):

- `FILE_ID.DIZ`: "Mine Bombers 3.11 Registered version / FREEWARE VERSION! MAY BE DISTRIBUTED"
- `MINEENG.TXT`, title box: "REGISTERED FREEWARE"
- `HISTORIA.TXT`, entry dated 31.12.2001: "Rekisteröity versio julkaistu freewarena." (the registered
  version was released as freeware)
- `MINEENG.TXT`, section 1, contradicts the above: "This is the REGISTERED version of MB. So it's against
  the law to copy or distribute it." This looks like text left over from before the freeware release, but
  it is still in the package.

The package contains no license text, no source code and no statement about modification, conversion or
embedding.

**If you are the rights holder or their representative**, please open an issue on this repository. Any
material of yours will be removed promptly on request.

### How comparable projects handle original game data

Checked in September 2026:

- OpenRCT2 requires the user's own copy of RollerCoaster Tycoon 2 (bought from Steam or GOG.com), which it
  is pointed at; the repository contains no original assets and the code is GPL-3.0 or later. Replacement
  graphics, music and scenarios live in separate projects.
- `idubrov/mb-reloaded` (the Rust project this one builds on) does not include the game data. Users
  point it at their own copy of Mine Bombers 3.11.
- ScummVM and OpenRA do not bundle proprietary data either. OpenRA offers a stripped-down download or
  imports content the user already owns. ScummVM expects the user's own copy and offers some freeware
  games from its own site (in at least one case with the author's explicit permission).
- OpenTyrian keeps the Tyrian 2.1 data (released as freeware "under no specific license") out of its
  source repository; the data comes as a separate download.
- OpenTTD avoids the question by shipping replacement assets (OpenGFX, OpenSFX, OpenMSX).

## 2. Source code

- `src/` and `crates/mb-core/` are derived from `idubrov/mb-reloaded` by Ivan Dubrov. That repository
  publishes no license (no `LICENSE` file, no license metadata), so by default all rights are reserved
  and this repository cannot relicense that code. A request to add a license is open at
  https://github.com/idubrov/mb-reloaded/issues/1.
- The "MIT" badge that used to be in the README had no basis and was removed.
- No license has been chosen yet for the code that was written for this repository (`crates/mb-wasm`,
  `web/`, `scripts/`). A license for the whole project depends on what the upstream author allows.

## 3. Bundled third-party code and binaries

- `web/vendor/chiptune3/` (chiptune3 0.8.9, MIT; its compiled libopenmpt is BSD) is vendored unmodified.
  Source, hashes and license are in `web/vendor/chiptune3/SOURCE.md` and its `LICENSE`.
- The DLLs in the repository root and in `lib/` (SDL2, SDL2_mixer, libgme, libxmp, libogg, libopus,
  libopusfile, libwavpack) are byte-for-byte identical to the official SDL2 2.30.8 and SDL2_mixer 2.8.0
  Windows packages. Their license texts are in `packaging/windows/licenses/` and ship in the release zip;
  provenance and hashes are in `packaging/windows/NOTICE.txt`.

## 4. Icon and placeholder art

The application icon (`res/minebombers.ico`, also used as `web/favicon.ico`) was drawn from geometric
primitives for this project. No original game art is used in it.

The PNG files in `assets/placeholders/` were procedurally generated specifically for this design bundle
from geometric primitives. No image file from Mine Bombers or another third-party game was used as a
source asset. They are development placeholders, not a statement about the licensing of the original
Mine Bombers game or any third-party property.
