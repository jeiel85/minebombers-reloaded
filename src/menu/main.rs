...
impl Application<'_> {
    ...
    loop {
      ...
      match scancode {
        ...
        Scancode::F11 => {
          ...
          self.render_main_menu(ctx, *selected)?;
          ctx.present()?;
        }
        Scancode::F1 => {
          ctx.set_window_scale(1)?;
          self.render_main_menu(ctx, *selected)?;
          ctx.present()?;
        }
        Scancode::F2 => {
          ctx.set_window_scale(2)?;
          self.render_main_menu(ctx, *selected)?;
          ctx.present()?;
        }
        Scancode::F3 => {
          ctx.set_window_scale(3)?;
          self.render_main_menu(ctx, *selected)?;
          ctx.present()?;
        }
        Scancode::F4 => {
          ctx.toggle_aspect_ratio()?;
          self.render_main_menu(ctx, *selected)?;
          ctx.present()?;
        }
        Scancode::Kp3 | Scancode::Return | Scancode::Return2 | Scancode::KpEnter => {
          if ctx.events.keyboard_state().is_scancode_pressed(Scancode::LAlt)
            || ctx.events.keyboard_state().is_scancode_pressed(Scancode::RAlt)
          {
            ctx.toggle_fullscreen()?;
            self.render_main_menu(ctx, *selected)?;
            ctx.present()?;
  ...
  fn render_main_menu(&self, ctx: &mut ApplicationContext, selected: SelectedMenu) -> Result<(), anyhow::Error> {
    ...
    ctx.with_render_context(|canvas| {
      ...
      self.font.render(canvas, 160, 460, palette[8], &hint)?;
