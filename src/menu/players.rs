...
struct State {
  ...
  active_player: u8,
  bot_difficulty: [Option<BotDifficulty>; 4],
}
...
impl Application<'_> {
  ...
  pub fn players_select_menu(
    ...
    let auto_bots = ctx.config.gameplay.auto_bots;
    ...
    if !exit {
      ...
      for idx in 0..total_players {
        ...
        let diff_opt = state.bot_difficulty[usize::from(idx)];
        selected.push(SelectedPlayer {
          name: state.roster.players[usize::from(roster_index)]
            ...
            .to_owned(),
          roster_index,
          is_bot: diff_opt.is_some(),
          bot_difficulty: diff_opt.unwrap_or(BotDifficulty::Medium),
        });
      }
    }
    Ok(selected)
  ...
  fn render_left_pane_names(&self, canvas: &mut WindowCanvas, state: &State) -> Result<(), anyhow::Error> {
    for player in 0..4i32 {
      ...
      if player < i32::from(state.players) {
        ...
        {
          let label = format!("{} {}", tag, stats.name);
          self.font.render(canvas, 120, player * 53 + 41, color, &label)?;
        }
      }
    }
